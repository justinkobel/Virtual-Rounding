...
$sharepointMasterListName = "VirtualRoundingLocationFavorites"
...
Add-PnPContentType -Name "VirtualRoundingLocationFavorite" -Group "VirtualRounding" | Out-Null
Start-Sleep -Seconds 5
$contentType = $null
while (!$contentType) {
    try {
        $contentType = Get-PnPContentType -Identity "VirtualRoundingLocationFavorite"
    }
    catch {
...
Add-PnpFieldToContentType -Field "ProviderEmail" -ContentType $contentType
...
Write-Host "Adding 'Location Favorite' Content Type to SharePoint List '$sharepointMasterListName'." -ForegroundColor Green
